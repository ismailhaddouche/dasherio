  ...
  template: `
    <div class="p-6">
      <header class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tótems</h1>
        <a 
          routerLink="new" 
      ...
      <div *ngIf="loading()" class="text-center py-8">
        ...
        <p class="mt-2 text-gray-600 dark:text-gray-400">Cargando tótems...</p>
      </div>
      ...
      <div *ngIf="!loading() && !error() && totems().length === 0" class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg">
        ...
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No hay tótems</h3>
        <p class="text-gray-600 dark:text-gray-400 mb-4">Crea tu primer tótem para empezar.</p>
        <a routerLink="new" class="text-primary hover:underline">Crear tótem →</a>
      </div>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr *ngFor="let totem of totems()" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  >
                    {{ totem.totem_type === 'STANDARD' ? 'Estándar' : 'Temporal' }}
                  </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 font-mono">
                  {{ totem.totem_qr || 'Sin QR' }}
                </td>
...
export class TotemListComponent implements OnInit {
  ...
  loadTotems(): void {
    ...
    this.totemService.getTotems().subscribe({
      ...
      error: (err) => {
        this.error.set(err.error?.message || 'Error al cargar tótems');
        this.loading.set(false);
      }
    });
  }
  ...
  deleteTotem(id: string, name: string): void {
    if (!confirm(`¿Eliminar tótem "${name}"? Esta acción no se puede deshacer.`)) return;
